 ...
 * <p>
 * 关于{@link ClassUtil#getGenericClass(Field, int...)}和 {@link ClassUtil#getGenericClass(TypeToken,
 * int...)}中参数pos的解释:
 * <p>
 * 考虑如下泛型定义:
 * <blockquote><pre>
...
public final class ClassUtil {
  ...
  private static <T> Set<Class<? extends T>> getClassesExtendClassUseReflections(
    ...
    return getClassesUseReflections(packagePath4Seek, isRecursive)
        .stream()
        .filter(selfClass -> Assert.notNull(selfClass) && !tUpperBoundClass.equals(selfClass)
            && ClassUtils.isAssignable(selfClass, tUpperBoundClass))
  ...
  private static <T> Set<Class<? super T>> getClassesSuperClassUseReflections(
    ...
    return getClassesUseReflections(packagePath4Seek, isRecursive)
        .stream()
        .filter(selfClass -> Assert.notNull(selfClass) && !tLowerBoundClass.equals(selfClass)
            && ClassUtils.isAssignable(tLowerBoundClass, selfClass))
  ...
  private static String filter(
    ...
    return isRecursive ? FilterBuilder.prefix(packagePath4Seek)
  ...
  private static Type getTypeWithPos(
    ...
    if (Assert.isNull(parameterizedType) || pos < 0) {
      return null;
    }
    try {
