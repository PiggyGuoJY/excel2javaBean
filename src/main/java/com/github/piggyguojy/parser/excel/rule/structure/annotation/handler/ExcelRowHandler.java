...
public final class ExcelRowHandler extends ExcelAnnotationHandler<ExcelRow> {
    ...
    private <E, G extends Collection<E>> void onFieldHandler(Class<G> gClass, ExcelRow excelRow, ExcelParser excelParser, Object ... args) {
        ...
        args[ANNOTATION_PARENT] = Assert.notNull(excelBeanParent) ? decideBiRule(excelRow, excelBeanParent, excelBeanParent.overrideRule()) : excelRow;
        ...
        Msg<Collection<E>> msg = onTypeHandler(rawClass, Assert.notNull(rawClass)?rawClass.getDeclaredAnnotation(ExcelRow.class):null, excelParser,args);
    ...
    private <G> Msg<Collection<G>> onTypeHandler(Class<G> gClass, ExcelRow excelRow, ExcelParser excelParser, Object ... args) {
        ...
        excelRow = Assert.notNull(excelRowParent) ? decideRule(excelRow, excelRowParent, excelRowParent.overrideRule()) : excelRow;
        if ( Assert.isNull(excelRow)) { return Msg.msg(new IllegalArgumentException(format("类型 %s 应该使用注解 %s 标注", gClass.getCanonicalName(), ExcelRow.class.getCanonicalName()))); }
        ...
        for (int rowIndex = excelRow.rowBegin(), expectantRowEnd = excelRow.rowEnd(), rowEnd = expectantRowEnd<0?Integer.MAX_VALUE:expectantRowEnd; rowIndex<=rowEnd; rowIndex++) {
            ...
            if ( Assert.isNull( row )) { log.warn("检测到空行, 停止解析..."); break; }
            // 准备泛型参数的实例
