...
public final class ExcelRowHandler
  ...
  protected Map<String, Object> getCustomerInheritableField() {
    return ExcelRowHandler.INHERITABLE_FIELD;
  }
  ...
  private <E, G extends Collection<E>> void onFieldHandler(
    ...
    args[ANNOTATION_PARENT] = notNull(excelBeanParent) ? decideBiRule(excelRow, excelBeanParent,
        excelBeanParent.overrideRule()) : excelRow;
    ...
    Msg<Collection<E>> msg = onTypeHandler(rawClass,
        notNull(rawClass) ? rawClass.getDeclaredAnnotation(ExcelRow.class) : null, excelParser,
        args);
    if (msg.isException()) {
      return;
    }
  ...
  private <G> Msg<Collection<G>> onTypeHandler(
    ...
    final ExcelRow finalExcelRow = notNull(excelRowParent) ? decideRule(excelRow, excelRowParent,
        excelRowParent.overrideRule()) : excelRow;
    if (isNull(finalExcelRow)) {
    ...
    final Sheet sheet = ExcelParser.ExcelParserHelper
        .decideSheet(finalExcelRow.sheet(), finalExcelRow.sheetName(), excelParser.getWorkbook());
    if (isNull(sheet)) {
    ...
    for (int rowIndex = finalExcelRow.rowBegin(), expectantRowEnd = finalExcelRow.rowEnd(), rowEnd =
        expectantRowEnd < 0 ? Integer.MAX_VALUE : expectantRowEnd; rowIndex <= rowEnd; rowIndex++) {
    ...
    private static <G> Map<String, Integer> getMapFromExcelRow(Class<G> gClass) {
      ...
      if (notNull(excelRow)) {
        ...
        if (map.matches(REGEX_EXCEL_ROW_MAP)) {
          ...
          int columnPointer = ExcelParser.ExcelParserHelper
              .decideColumnNo(excelRow.columnNameBegin(), excelRow.columnBegin());
