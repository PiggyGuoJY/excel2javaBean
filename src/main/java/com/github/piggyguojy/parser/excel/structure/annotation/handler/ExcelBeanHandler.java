...
public final class ExcelBeanHandler extends ExcelAnnotationHandler<ExcelBean> {
  ...
  public <G> Msg<?> onType(
    ...
    final ExcelBean finalExcelBean =
        notNull(nestedParent) ? decideAnnotationRule(excelBean, nestedParent, INHERITABLE_FIELD,
            nestedParent.overideRule()) : excelBean;
    if (notNull(nestedParent) && nestedParent.stepBy() > 0) {
      ClassUtil.changeAnnotationFieldValue(finalExcelBean, "sheet",
          finalExcelBean.sheet() + nestedParent.stepBy());
    }
    // 3.当根据sheet和sheetName不能判断Sheet页时, 停止解析
    if (isNull(ExcelParser.ExcelParserHelper
        .decideSheet(finalExcelBean.sheet(), finalExcelBean.sheetName(),
    ...
    Stream.of(gClass.getDeclaredFields())
        .parallel()
        // 处理含有特定注解的属性
        .filter(fieldSelf -> notNull(
            ClassUtil.getTheOnlyOneAnnotation(fieldSelf, ANNOTATIONS_ON_FIELD)))
        ...
        .forEach(fieldSelf -> {
          Class<? extends Annotation> annotationClass = ClassUtil
              .getTheOnlyOneAnnotation(fieldSelf, ANNOTATIONS_ON_FIELD);
          // 具体根据注解分发
    ...
    Stream.of(gClass.getDeclaredFields())
        ...
        .forEach(fieldSelf -> {
          ...
          Msg<?> msg = getAnnotationHandlerRegistered(ExcelBean.Nested.class).onField(
              fieldSelf.getType(), fieldSelf.getDeclaredAnnotation(ExcelBean.Nested.class),
              excelParser,
              immutableArgs[ARGS_INIT],
              immutableArgs[VALUE_RETURNED],
              g,
