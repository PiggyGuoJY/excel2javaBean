 ...
 * <td style="background-color:khaki">{@link BigInteger#ONE}({@code true})或{@link
 * BigInteger#ZERO}({@code false})</td>
 * </tr>
 * <tr>
 * <td>Float</td>
...
public class ExcelTransformerRuleTypeAdvanced
  ...
  private static <T> T template(
    ...
    switch (cell.getCellType()) {
      case NUMERIC:
        return numeric.apply(cell);
      case STRING:
        return string.apply(cell);
      case FORMULA:
        return formula.apply(
            cell.getSheet().getWorkbook().getCreationHelper().createFormulaEvaluator()
                .evaluateInCell(cell));
      case BLANK:
        return blank.apply(cell);
      case BOOLEAN:
        return bool.apply(cell);
      case ERROR:
        return error.apply(cell);
      default:
        return null;
    }
  }
  ...
  protected Character cell2Character(Cell cell) {
    return template(
        cell,
        c -> null,
        c -> c.getStringCellValue().length() == 0 ? Character.MIN_VALUE
            : c.getStringCellValue().charAt(0),
        this::cell2Character,
