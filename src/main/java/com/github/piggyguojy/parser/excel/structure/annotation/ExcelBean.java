 ...
 * 复合类型
 *
 * 备注: 1. 类属性应该使用{@code ExcelCell}, {@code ExcelColumn}, {@code ExcelRow}或{@code
 * ExcelBean.Nested}标注, 否则将跳过改属性 2. 使用{@code ExcelBean.Skip}修饰的类属性的效果同1的否定情况
 *
 * <p> 创建时间：2018/10/29
 *
...
public @interface ExcelBean {
  ...
  /**
   * @return 本JavaBean是否允许作为属性被嵌套在其他使用{@code ExcelBean}注解标注的JavaBean中, 默认不允许; 本注解影响{@code
   * ExcelBean.Nested}的使用
   */
   ...
   * 申明特定属性是内嵌的
   *
   * 备注: 1. 该注解要求必须和{@code ExcelBean}同时出现(单独出现不生效) 2. 被本注解标注的属性的类型定义必须使用{@code
   * ExcelBean}标注且其isNestable属性为true(否则本注解不生效) 3. 由形式 (@ExcelBean)bean{ (@ExcelBean.Nested)field} ~
   * (@ExcelBean)bean2{ (@ExcelBean.Nested)field} ~ ... 确定的关系将形成一条解析链, 相同的解析过程将递归调用
   *
   * <p> 创建时间：2019/2/13
   *
   * @author <a href="https://github.com/PiggyGuoJY" target="_blank">PiggyGuoJY</a>
