...
public abstract class AbstractAnnotationHandler<A extends Annotation, P extends AbstractParser>
  ...
  private static final Map<Class<? extends Annotation>, AbstractAnnotationHandler> MAP_ANNOTATION_HANDLER = Maps
      .newHashMapWithExpectedSize(8);
  ...
  static {
    ...
    log.info("excel2javaBean初始化完成, 用时 {} ms, 获取到对以下注解的支持:\n {}",
        finish - begin,
        MAP_ANNOTATION_HANDLER.keySet().parallelStream().map(Class::getSimpleName)
          ...
          .forEach(
              selfClass -> {
                ...
                if (className.matches("^.+(\\.[A-Z_$][a-zA-Z_$]*){2}$")) {
                  className = className
                      .replaceAll("(^.+\\.[A-Z_$][a-zA-Z_$]*)\\.([A-Z_$][a-zA-Z_$]*$)", "$1\\$$2");
                }
                try {
  ...
  public static class AbstractAnnotationHandlerHelper {
    ...
    public static <A extends Annotation, B extends Annotation> A decideAnnotationRule(
      ...
      for (Map.Entry entry : stringObjectMap.entrySet()) {
        a = decideAnnotationRule(a, parent, (String) entry.getKey(), entry.getValue(),
            overrideRule);
        if (isNull(a)) {
          log.error("处理 {} 时出错, 中断执行并返回 {}", entry.getKey(), "son");
          return son;
        }
      }
      return a;
    ...
    static <G, A extends Annotation, B extends Annotation> A decideAnnotationRule(
      ...
      try {
        ...
        if (sonMap.get(fieldName).getClass().equals(gDefaultValue.getClass()) && parentMap
            .get(fieldName).getClass().equals(gDefaultValue.getClass())) {
      ...
      if (isNull(parentGValue)) {
        ...
        if (isNull(sonGValue)) {
          ...
          switch (overrideRule) {
            case PARENT_FIRST:
              if (!parentGValue.equals(gDefaultValue)) {
                sonMap.put(fieldName, parentGValue);
              }
              break;
            case PARENT_FORCE:
              sonMap.put(fieldName, parentGValue);
              break;
            case SON_FIRST:
              if (sonGValue.equals(gDefaultValue)) {
                sonMap.put(fieldName, parentGValue);
              }
              break;
            case SON_FORCE:
      ...
      sonMemberValues.setAccessible(false);
      parentMemberValues.setAccessible(false);
      return son;
    }
  }
}
