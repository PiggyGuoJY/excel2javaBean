...
public interface BiInheritableRule<X, Y> {
   ...
   * 按如下方式进行规则覆盖
   *
   * 子: 有配置, 父: 有配置 = 父 子: 无配置, 父: 有配置 = 父 子: 有配置, 父: 无配置 = 子 子: 无配置, 父: 无配置 = 父
   *
   * @param son 子规则
   * @param parent 父规则
   * @return 最终子规则
   ...
   * 按如下方式进行规则覆盖
   *
   * 子: 有配置, 父: 有配置 = 子 子: 无配置, 父: 有配置 = 父 子: 有配置, 父: 无配置 = 子 子: 无配置, 父: 无配置 = 子
   *
   * @param son 子规则
   * @param parent 父规则
   * @return 最终子规则
  ...
  default X decideBiRule(X son, Y parent, OverrideRule overrideRule) {
    switch (overrideRule) {
      case PARENT_FIRST:
        return decideBiRuleOnParentFirst(son, parent);
      case PARENT_FORCE:
        return decideBiRuleOnParentForce(son, parent);
      case SON_FIRST:
        return decideBiRuleOnSonFirst(son, parent);
      case SON_FORCE:
        return decideBiRuleOnParentForce(son, parent);
      default:
        return decideBiRuleOnParentForce(son, parent);
