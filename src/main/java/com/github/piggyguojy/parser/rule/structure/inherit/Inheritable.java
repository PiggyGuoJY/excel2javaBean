...
public interface Inheritable<X> {
  ...
  default X decideRule(X son, X parent, OverrideRule overrideRule) {
    switch (overrideRule) {
      case PARENT_FIRST:
        return decideRuleOnParentFirst(son, parent);
      case PARENT_FORCE:
        return decideRuleOnParentForce(son, parent);
      case SON_FIRST:
        return decideRuleOnSonFirst(son, parent);
      case SON_FORCE:
        return decideRuleOnSonForce(son, parent);
      default:
        return decideRuleOnParentForce(son, parent);
    }
  }
