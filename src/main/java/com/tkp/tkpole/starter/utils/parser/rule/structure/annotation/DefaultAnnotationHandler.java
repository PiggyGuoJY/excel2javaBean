package com.tkp.tkpole.starter.utils.parser.rule.structure.annotation;
...
public final class DefaultAnnotationHandler extends AbstractAnnotationHandler{
    ...
    @Override
    public Msg<?> onType(Class aClass, Annotation annotation, AbstractParser abstractParser, Object... args) { return msg(new IllegalStateException(format("没有找到对应注解 %s 的处理器", annotation.getClass().getName()))); }
    @Override
    public Msg<?> onField(Class aClass, Annotation annotation, AbstractParser abstractParser, Object... args) { return msg(new IllegalStateException(format("没有找到对应注解 %s 的处理器", annotation.getClass().getName()))); }
}