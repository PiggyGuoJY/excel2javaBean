...
public final class ExcelBeanHandler extends ExcelAnnotationHandler<ExcelBean> {
    ...
    public <G> Msg<?> onType(Class<G> gClass, ExcelBean excelBean, ExcelParser excelParser, Object ... args) {
        // 1.实例化容器
        final G g;
        try { g = gClass.newInstance(); } catch ( IllegalAccessException | InstantiationException e) { log.error( e.getMessage(), e); return msg( e); }
        ...
        if ( isNull(ExcelParser.ExcelParserHelper.decideSheet(finalExcelBean.sheet(),finalExcelBean.sheetName(),excelParser.getWorkbook()))) { return msg(); }
        // 4.保存不可变参数, 防止被篡改
