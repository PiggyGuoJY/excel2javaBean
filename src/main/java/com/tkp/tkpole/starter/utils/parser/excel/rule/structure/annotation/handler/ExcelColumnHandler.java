...
public final class ExcelColumnHandler extends ExcelAnnotationHandler<ExcelColumn> {
    ...
    private <G> Msg<Collection<G>> onTypeHandler(Class<G> gClass, ExcelColumn excelColumn, ExcelParser excelParser, Object ... args) {
        if ( isNull(gClass)) { return msg(new IllegalStateException("无法获取容器泛型参数"));}
        ...
        if ( isNull(excelColumn)) { return msg(new IllegalArgumentException(format("类型 {} 应该使用注解 {} 标注", gClass.getCanonicalName(), ExcelColumn.class.getCanonicalName()))); }
...
//            // todo ... 也可以不这么停止, 具体可以看以后的情况
            ...
            if ( isNull( gInstance)) {
                ...
                return msg(new IllegalStateException("无法通过反射实例化泛型参数的实例"));
            }
        ...
        return msg((Collection<G>) objectCollection);
    }
